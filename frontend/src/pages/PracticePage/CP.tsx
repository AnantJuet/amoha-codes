...
const SHEET_NAME = "CompetitiveProgramming";
interface Problem {
  id: string | number;
  title: string;
  link: string;
...
const CP: FC<CPProps> = ({ darkMode }) => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      if (!token) {
        // Reset all problems to not solved
        setAllProblems(JSON.parse(JSON.stringify(initialProblemsByRating)) as ProblemsByRating);
        return;
      }
      try {
        const response = await API.get(`/question/status?sheet=${SHEET_NAME}`);
        const solvedIds: string[] = response.data;
        // Update all ratings with solved status
        setAllProblems((prev) => {
          const updated: ProblemsByRating = {};
          for (const rating of allRatings) {
            updated[rating] = (prev[rating] || []).map((p) => ({
              ...p,
              status: solvedIds.includes(`${rating}-${p.id}`),
            }));
          }
          return updated;
        });
  ...
  const handleProblemStatusChange = async (problemId: string | number) => {
    const current = allProblems[selectedRating];
    if (!current) return;

    const problem = current.find((p) => p.id === problemId);
    if (!problem) return;

    const newStatus = !problem.status;

    // Optimistically update UI
    setAllProblems((prev) => {
      const currentList = prev[selectedRating];
      if (!currentList) return prev;
      const updated = currentList.map((p) => (p.id === problemId ? { ...p, status: newStatus } : p));
    ...
    if (token) {
      try {
        await API.post("/question/check", {
          question_id: `${selectedRating}-${problemId}`,
          title: problem.title,
          category: `Rating ${selectedRating}`,
          difficulty: selectedRating <= 1000 ? "Easy" : selectedRating <= 1300 ? "Medium" : "Hard",
          company: null,
