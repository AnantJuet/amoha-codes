...
const SHEET_NAME = "ProgrammingLang";
...
const ProgrammingLang: React.FC<{ darkMode: boolean }> = ({ darkMode }) => {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      try {
        ...
        setCompletedLevels(new Set(solvedIds.map(Number)));
      } catch (error) {
  ...
  const toggleComplete = async (id: number) => {
    const level = DUMMY_LEVELS.find((l) => l.id === id);
    if (!level) return;

    const isCurrentlyCompleted = completedLevels.has(id);
    ...
    if (token) {
      try {
        ...
        setCompletedLevels((prev) => {
          const newSet = new Set(prev);
          if (newCompletedState) newSet.delete(id);
          else newSet.add(id);
          return newSet;
        });
      }
