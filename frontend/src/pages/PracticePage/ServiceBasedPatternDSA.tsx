...
const SHEET_NAME = "ServiceBasedPatternDSA Sheet";

// Helper function to create a unique string ID for each problem
let problemIdCounter = 0;
const createInitialState = () => {
  problemIdCounter = 0;
  return dsaPatterns.map((pattern, patternIndex) => ({
    ...pattern,
    subcategories: pattern.subcategories.map((subcat, subcatIndex) => ({
      ...subcat,
      problems: subcat.problems.map((problem, problemIndex) => ({
        ...problem,
        id: `${patternIndex}-${subcatIndex}-${problemIndex}`,
        numericId: problemIdCounter++,
        solved: false,
      })),
    })),
  }));
...
function ServiceBasedPatternDsa({ darkMode = false }) {
  ...
  useEffect(() => {
    ...
    // Listen for auth changes to reload progress
  ...
  const handleCheckboxChange = async (problemId: string, category: string, subcategory: string) => {
    // Find the problem to get current state
    let currentProblem: { id: string; title: string; solved: boolean } | undefined;
    for (const pattern of patternsList) {
      if (pattern.category === category) {
        for (const subcat of pattern.subcategories) {
          if (subcat.subcategory === subcategory) {
            currentProblem = subcat.problems.find((p) => p.id === problemId);
            break;
          }
        }
      }
            ...
            subcategories: pattern.subcategories.map((subcat) => {
              if (subcat.subcategory === subcategory) {
                return {
                  ...subcat,
                  problems: subcat.problems.map((problem) =>
                    problem.id === problemId ? { ...problem, solved: newSolvedState } : problem
                  ),
                };
              }
              return subcat;
