...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/DatabaseLanguages/plsqlData";

interface PlsqlTutorialProps {
  darkMode: boolean;
}
...
const PlsqlTutorial: React.FC<PlsqlTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "plsql_home.asp");
