...
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/DatabaseLanguages/NoSQL/nosqlData";

interface NoSQLTutorialProps {
  darkMode: boolean;
}
...
const NoSQLTutorial: React.FC<NoSQLTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "nosql_home.asp");
