...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/DatabaseLanguages/MongoDB/mongodbData";

interface MongoDBTutorialProps {
  darkMode: boolean;
}
...
const MongoDBTutorial: React.FC<MongoDBTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "mongodb_home");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="MongoDB Tutorial"
      />
