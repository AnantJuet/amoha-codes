...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/DatabaseLanguages/tsqlData";

interface TsqlTutorialProps {
  darkMode: boolean;
}
...
const TsqlTutorial: React.FC<TsqlTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "tsql_home.asp");
