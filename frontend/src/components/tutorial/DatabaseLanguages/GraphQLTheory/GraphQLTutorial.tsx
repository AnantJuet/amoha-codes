...
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/DatabaseLanguages/GraphQL/graphqlData";

interface GraphQLTutorialProps {
  darkMode: boolean;
}

// Reusable logic to flatten data for navigation
...
const GraphQLTutorial: React.FC<GraphQLTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "graphql_home.asp");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="GraphQL Tutorial"
      />
...
export default GraphQLTutorial;
