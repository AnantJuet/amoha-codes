...
import { type TopicContent } from "../../../../data/tutorial/DatabaseLanguages/GraphQL/graphqlData";
...
const MainContent: React.FC<MainContentProps> = ({ topic, darkMode }) => {
  ...
  return (
    ...
    >
      ...
      {topic.subtitle && (
        <p
          className={`text-xl mt-2 text-gray-600 ${darkMode ? "text-gray-400" : "text-gray-600"}`}
        >
          {topic.subtitle}
        </p>
      )}
      <div className={`prose max-w-none dark:prose-invert ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
        {topic.content}
      </div>
    </div>
  );
