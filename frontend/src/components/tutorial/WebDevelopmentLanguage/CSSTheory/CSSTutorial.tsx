...
import { tutorialData, tutorialContent, type SidebarItem } from "./CSSData";


interface CSSTutorialProps {
 darkMode: boolean;
}
...
const CSSTutorial: React.FC<CSSTutorialProps> = ({ darkMode }) => {

  // eslint-disable-next-line react-hooks/rules-of-hooks
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "css_home.asp");
