...
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/TrendingTech/SpringBootData/springBootData";

interface SpringBootTutorialProps {
  darkMode: boolean;
}
...
const SpringBootTutorial: React.FC<SpringBootTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "sb_home");
...
export default SpringBootTutorial;
