...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/TrendingTech/Kubernetes/kubernetesData";

interface KubernetesTutorialProps {
  darkMode: boolean;
}
...
const KubernetesTutorial: React.FC<KubernetesTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "k8s_home");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="Kubernetes Tutorial"
      />
