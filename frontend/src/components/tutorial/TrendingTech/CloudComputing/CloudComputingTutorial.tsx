...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/TrendingTech/cloudComputingData";

interface CloudComputingTutorialProps {
  darkMode: boolean;
}
...
const CloudComputingTutorial: React.FC<CloudComputingTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "cloud_home.asp");
...
export default CloudComputingTutorial;
