...
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/TrendingTech/EthicalHackingData/ethicalHackingData";

interface EthicalHackingTutorialProps {
  darkMode: boolean;
}
...
const EthicalHackingTutorial: React.FC<EthicalHackingTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "eh_home");
...
export default EthicalHackingTutorial;
