...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/TrendingTech/awsData";

interface AWSTutorialProps {
  darkMode: boolean;
}
...
const AWSTutorial: React.FC<AWSTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "aws_home.asp");
