...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/TrendingTech/UnixData/unixData";

interface UnixTutorialProps {
  darkMode: boolean;
}
...
const UnixTutorial: React.FC<UnixTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "unix_home");
