...
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/TrendingTech/SDLCData/sdlcData";

interface SDLCTutorialProps {
  darkMode: boolean;
}
...
const SDLCTutorial: React.FC<SDLCTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "sdlc_home");
