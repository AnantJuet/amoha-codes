...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/TrendingTech/Docker/dockerData";

interface DockerTutorialProps {
  darkMode: boolean;
}
...
const DockerTutorial: React.FC<DockerTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "docker_home");
