...
import { tutorialContent, tutorialData, type SidebarItem } from "../../../../data/tutorial/WebDevelopment/WebAssembly/wasmData";

interface WebAssemblyTutorialProps {
  darkMode: boolean;
}
...
const WebAssemblyTutorial: React.FC<WebAssemblyTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "wasm_home");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="WebAssembly Tutorial"
      />
...
export default WebAssemblyTutorial;
