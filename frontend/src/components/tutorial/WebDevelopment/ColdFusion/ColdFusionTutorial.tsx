...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/WebDevelopment/ColdFusion/coldfusionData";

interface ColdFusionTutorialProps {
  darkMode: boolean;
}
...
const ColdFusionTutorial: React.FC<ColdFusionTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "cf_home");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="ColdFusion Tutorial"
      />
...
export default ColdFusionTutorial;
