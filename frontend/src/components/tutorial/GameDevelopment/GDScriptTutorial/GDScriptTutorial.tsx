...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/GameDevelopment/GDScript/gdscriptData";

interface GDScriptTutorialProps {
  darkMode: boolean;
}
...
const GDScriptTutorial: React.FC<GDScriptTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "gdscript_home");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="GDScript Tutorial"
      />
...
export default GDScriptTutorial;
