...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/SystemsProgramming/AdaProgramming/adaData";

interface AdaTutorialProps {
  darkMode: boolean;
}
...
const AdaTutorial: React.FC<AdaTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "ada_home");
