...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/SystemsProgramming/ZigProgramming/zigData";

interface ZigTutorialProps {
  darkMode: boolean;
}
...
const ZigTutorial: React.FC<ZigTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "zig_home");
