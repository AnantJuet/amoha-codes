...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/SystemsProgramming/AssemblyLanguage/assemblyData";

interface AssemblyTutorialProps {
  darkMode: boolean;
}
...
const AssemblyTutorial: React.FC<AssemblyTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "assembly_home");
