...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/SystemsProgramming/DLanguage/dData";

interface DLanguageTutorialProps {
  darkMode: boolean;
}
...
const DLanguageTutorial: React.FC<DLanguageTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "d_home");
  ...
  return (
    <div className={`flex h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <Sidebar
        ...
        darkMode={darkMode}
        tutorialTitle="D Language Tutorial"
      />
...
export default DLanguageTutorial;
