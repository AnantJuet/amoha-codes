...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/ScriptingLanguages/perlData";

interface PerlTutorialProps {
  darkMode: boolean;
}
...
const PerlTutorial: React.FC<PerlTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "perl_home");
