...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/ScriptingLanguages/luaData";

interface LuaTutorialProps {
  darkMode: boolean;
}
...
const LuaTutorial: React.FC<LuaTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "lua_home");
