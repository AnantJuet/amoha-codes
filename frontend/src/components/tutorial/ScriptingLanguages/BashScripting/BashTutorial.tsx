...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/ScriptingLanguages/bashData";

interface BashTutorialProps {
  darkMode: boolean;
}
...
const BashTutorial: React.FC<BashTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "bash_home");
