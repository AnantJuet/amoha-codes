...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/ScriptingLanguages/groovyData";

interface GroovyTutorialProps {
  darkMode: boolean;
}
...
const GroovyTutorial: React.FC<GroovyTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "groovy_home");
