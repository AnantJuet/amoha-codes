...
import { tutorialData, tutorialContent, type SidebarItem } from "../../../../data/tutorial/ScriptingLanguages/powershellData";

interface PowerShellTutorialProps {
  darkMode: boolean;
}
...
const PowerShellTutorial: React.FC<PowerShellTutorialProps> = ({ darkMode }) => {
  const flatHrefs = useMemo(() => flattenData(tutorialData), []);
  const [activeHref, setActiveHref] = useState(flatHrefs.length > 0 ? flatHrefs[0] : "ps_home");
...
export default PowerShellTutorial;
